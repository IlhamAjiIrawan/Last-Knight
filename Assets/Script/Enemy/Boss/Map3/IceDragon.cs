...
public class IceDragon : MonoBehaviour
{
    ...
    public float skill1Range = 6f;       
    public float skill1Damage = 20f; // Damage per satu kali cakar (Total 40 jika kena dua-duanya)
    public float skill1Width = 4f;        
    public float clawRange = 4.5f; // Jangkauan kedepan cakaran
    [Tooltip("Total durasi boss terdiam saat melangsungkan seluruh gerakan skill 1")]
    public float skill1TotalDuration = 2f; 
    ...
    {
        Debug.LogWarning("INTERRUPT: Ice Boss memotong semua aksi untuk mengaktifkan Counter Attack!");
    ...
    IEnumerator Skill1Routine()
    {
        ...
        // Munculkan indikator Danger Zone kotak
        if (dangerZoneBoxPrefab != null)
        {
            Vector3 boxCenter = transform.position + transform.forward * (clawRange / 2f);
            ...
            currentSkillDangerZone.transform.localScale = new Vector3(skill1Width, clawRange, 1f);
        }

        // Jalankan animasi mencakar
        anim.SetTrigger("skill1");

        // Biarkan Coroutine berjalan menahan state boss selama durasi penuh gerakan animasi mencakar
        yield return new WaitForSeconds(skill1TotalDuration);

        // Pengaman: Jika indikator belum hancur karena event terlewati, hancurkan di sini
        if (currentSkillDangerZone != null) Destroy(currentSkillDangerZone);
    ...
    public void IceDragon_ExecuteClawDamage()
    {
        if (isDead) return;

        // Begitu cakar pertama mengayun (Event pertama dipicu), hapus danger zone merahnya
        if (currentSkillDangerZone != null) Destroy(currentSkillDangerZone);

        // Kalkulasi area hit cakar menggunakan OverlapBox
        Vector3 boxCenter = transform.position + transform.forward * (clawRange / 2f);
        Vector3 boxHalfExtents = new Vector3(skill1Width / 2f, 2f, clawRange / 2f);
        ...
        foreach (Collider col in hitColliders)
        {
            if (col.CompareTag("Player"))
            {
                ...
                Debug.Log("<color=cyan>❄️ [IceDragon Event]: Player terkena cakar es!</color>");
                break; // Keluar dari loop jika player sudah terkena hit cakar ini
            }
        }
    }
    ...
    IEnumerator Skill2Routine()
    {
        ...
        // Hit damage instan ke arah cone
        ...
        if (iceFloorPrefab != null && !isDead)
        {
            ...
            {
                // Jika kamu memasang spawn point khusus (misal di depan mulut naga atau titik target di tanah)
                spawnIcePos = iceSpawnPoint.position;
                spawnIceRot = iceSpawnPoint.rotation;
            }
            else
            {
                // Antisipasi/Fallback: Jika spawn point kosong di Inspector, es otomatis muncul di tanah posisi player
    ...
    IEnumerator CounterAttackRoutine()
    {
        ...
        {
            Transform spawnPoint = shieldSpawnPoint != null ? shieldSpawnPoint : transform;
            currentSkillVFX = Instantiate(blizzardPrefab, spawnPoint.position, spawnPoint.rotation);
    ...
    void OnDrawGizmosSelected()
    {
        ...
        Vector3 localCenter = new Vector3(0f, 0f, clawRange / 2f);
        Vector3 localSize = new Vector3(skill1Width, 0.1f, clawRange);
